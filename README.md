...
- Bucket tooltips show the actual fish, and bucket icons get two color
  squares so a chest of fish reads at a glance
- Fish collection screen (B): collected variants and catch counts per
  server. Hovering buckets in chests fills in what you already own
- First catch of a variant pops a toast with the live fish and its number;
  completing a pattern, the commons, the solids or the dex celebrates bigger
